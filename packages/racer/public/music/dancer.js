    ...
    load : function ( path ) {
      ...
      req.onload = function () {
        ...
        if ( _this.context.decodeAudioData ) {
          // modern browsers return a promise, older ones only call the callbacks
          var decoding = _this.context.decodeAudioData( req.response, onDecoded, function ( e ) {
            console.log( e );
          });
          if ( decoding && decoding.then ) {
            decoding.then( onDecoded, function ( e ) {
              console.log( e );
            });
          }
        } else {
          onDecoded( _this.context.createBuffer( req.response, false ) );
        }
      };
      req.send();

      var createProcessor = this.context.createScriptProcessor || this.context.createJavaScriptNode;
      this.proc = createProcessor.call( this.context, SAMPLE_SIZE / 2, 1, 1 );
